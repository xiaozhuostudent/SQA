...
import { getTeacherLogs } from '@/api/log'
...
const loadLogs = async () => {
  ...
  try {
    ...
    const res = await getTeacherLogs(params)
    logs.value = res.data.records
    pagination.total = res.data.total
    syncModuleOptions(res.data.records)
  } catch (error) {
    console.error('加载教师日志失败:', error)
    ElMessage.error('加载教师日志失败: ' + (error.response?.data?.message || error.message))
  } finally {
    loading.value = false
  }
}
