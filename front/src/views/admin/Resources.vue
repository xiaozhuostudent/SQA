          ...
          >
            ...
            <el-table-column label="操作" fixed="right" width="280">
              <template #default="scope">
                ...
                <el-button 
                  v-if="hasPermission('resource:approve')"
                  size="small" 
                  type="primary" 
                ...
                <el-button 
                  v-if="hasPermission('resource:delete')"
                  size="small" 
                  type="danger" 
 ...
 * 
 * 权限说明：
 * - 管理员：可以查看、上传、编辑、删除所有资源，预览所有资源
 * - 教师：可以上传、编辑/删除自己的资源（在教师端）
 * - 学生：只能查看和预览已审核的资源（在学生端）
 */
import { ref, computed, onMounted } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { 
  Folder, Document, VideoPlay, Tickets, Files, Upload, FullScreen
...
const filteredResources = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    filtered = filtered.filter(r => 
      ...
      r.uploaderName?.toLowerCase().includes(keyword) ||
...
const handleUpload = async () => {
  ...
  try {
    // 获取选中课程的名称
    const selectedCourse = allCourses.value.find(c => c.id === resourceForm.value.courseId)
    ...
    const data = {
      ...
      uploaderName: userStore.user?.name || '管理员',
