      ...
      <div v-if="currentDiscussion" class="discussion-detail">
        ...
        <div v-for="reply in replyList" :key="reply.id" class="reply-item">
          ...
          <div class="reply-actions">
            ...
            <el-button v-if="userStore.userInfo?.id && !reply.isAccepted && currentDiscussion.authorId === userStore.userInfo.id" 
              size="small" type="success" text @click="acceptReply(reply)">
              ✓ 采纳
            </el-button>
...
const createDiscussion = async () => {
  ...
  try {
    // 将role从数字转换为字符串
    let authorRole = 'teacher'
    if (userStore.userInfo?.role) {
      const roleMap = { 1: 'student', 2: 'teacher', 3: 'admin', 4: 'admin' }
      authorRole = typeof userStore.userInfo.role === 'number' 
    ...
    await discussionApi.createDiscussion({
      ...
      authorName: userStore.userInfo?.realName || '教师',
...
const viewDiscussion = async (row) => {
  ...
  console.log('当前登录用户ID:', userStore.userInfo?.id, '姓名:', userStore.userInfo?.realName)
  ...
  try {
    ...
    replyList.value.forEach(reply => {
      console.log(`回复ID: ${reply.id}, 作者ID: ${reply.authorId}, 作者姓名: ${reply.authorName}`)
    })
...
const acceptReply = async (reply) => {
  try {
    await discussionApi.acceptReply(currentDiscussion.value.id, reply.id)
    ElMessage.success('采纳成功')
    reply.isAccepted = true
    currentDiscussion.value.isResolved = true
  } catch (error) {
