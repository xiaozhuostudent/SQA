  ...
  <div class="student-log-table">
    ...
    <el-form :inline="true" :model="searchForm" class="search-form">
      <el-form-item label="学生姓名">
        <el-input v-model="searchForm.studentName" placeholder="学生姓名" clearable style="width: 150px" />
      </el-form-item>
...
import { getStudentLogs } from '@/api/log'
...
const loadLogs = async () => {
  ...
  try {
    ...
    const res = await getStudentLogs(params)
    logs.value = res.data.records
    pagination.total = res.data.total
    syncModuleOptions(res.data.records)
  } catch (error) {
    console.error('加载学生日志失败:', error)
    ElMessage.error('加载学生日志失败: ' + (error.response?.data?.message || error.message))
  } finally {
    loading.value = false
  }
}
